document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        ...
        document.getElementById('shipmentForm').addEventListener('submit', async function(event) {
            ...
            // Validación de datos
            const cliente_documento = document.getElementById('cliente_documento').value.trim();
            const cliente_nombre = document.getElementById('cliente_nombre').value.trim();
            const cliente_direccion = document.getElementById('cliente_direccion').value.trim();
            const cliente_telefono = document.getElementById('cliente_telefono').value.trim();
            const cliente_correo = document.getElementById('cliente_correo').value.trim();
            const direccion_destino = document.getElementById('direccion_destino').value.trim();
            const costo_envio = parseFloat(document.getElementById('costo_envio').value.trim());
            const estado_envio = document.getElementById('estado_envio').value.trim();
  
            if (!cliente_documento || !cliente_nombre || !cliente_direccion || !cliente_telefono || !cliente_correo || !direccion_destino || isNaN(costo_envio)) {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Todos los campos son obligatorios y deben tener el formato correcto.',
                });
                return;
            }
  
            const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            if (!emailPattern.test(cliente_correo)) {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
            ...
            const productos = Array.from(document.getElementsByName('producto_id[]')).map((select, index) => {
                return {
                    ...
                    cantidad: parseInt(document.getElementsByName('cantidad[]')[index].value.trim())
                };
            });
  
            for (const producto of productos) {
                if (isNaN(producto.producto_id) || isNaN(producto.cantidad) || producto.cantidad <= 0) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'La cantidad del producto deben ser números válidos y mayores que 0.',
                    });
                    return;
                }
            }
        ...
        document.getElementById('searchEnvioButton').addEventListener('click', async function() {
            const envioId = document.getElementById('searchEnvioId').value.trim();
            try {
                const response = await axios.get(`/shipments/${envioId}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
